...
import {ServiceDumpsters,type DumpsterCancellationDetails,type DumpsterRow} from "@/components/service-dumpsters";
...
export default async function Detalhe({params}:{params:Promise<{id:string}>}){
 ...
 const cancellationDetails:Record<string,DumpsterCancellationDetails>={};
 for(const event of events)if(event.event_type==="dumpster_cancelled"&&event.dumpster_id&&!cancellationDetails[event.dumpster_id])cancellationDetails[event.dumpster_id]={cancelledAt:event.occurred_at,reason:event.reason,cancelledBy:event.performed_by_name};
 return <AppShell title={`Serviços / ${service.service_number}`}>
  ...
  <ServiceDumpsters serviceId={service.id} dumpsters={dumpsters} isAdmin={isAdmin} serviceStatus={service.status} cancellationDetails={cancellationDetails}/>
